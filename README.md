...
По умолчанию `uninstall.sh` удаляет listener и автозапуск, но оставляет
`~/.config/x11-layout-switch-release.conf`. Это сделано специально, чтобы не
терять пользовательские настройки. Если конфиг тоже нужно удалить, используйте
`--purge-config`.
